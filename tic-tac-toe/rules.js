...
function begin_play(){
	...
	if (isEmpty(document.getElementById('player1_id').value) || isEmpty(document.getElementById('player2_id').value)){
		// raise alert
		alert("Game requires two players. Please enter two names.");
		return;
	}
	else{
		// disable name fields
		document.getElementById('player1_id').value = document.getElementById('player1_id').value.concat(' (X)');
		document.getElementById('player2_id').value = document.getElementById('player2_id').value.concat(' (O)');
		//add x/o
...
*/
function reset_play(){
	// clear names and move text boxes
...
function play() {
	...
	if (move == "A1" || move == "A2" || move == "A3"){
		// find correct value of elem
		if (move == "A1"){elem = 0};
		if (move == "A2"){elem = 1};
		if (move == "A3"){elem = 2};

		// ensure move hasn't been taken yet
		if (board_state[elem] == -1){
			...
			alert("Spot already taken, pick again!");
			return;
		}
	}
